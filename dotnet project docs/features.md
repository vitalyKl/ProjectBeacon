...
- **FR-A2.** Beacon генерирует конфигурацию `llama-swap` (`config.yaml`) из этого реестра и (пере)запускает/reload'ит `llama-swap` при изменении реестра. Бэкенды с флагом `Concurrent` («Load with others») попадают в `groups.resident` (`swap: false`, `persistent: true`); остальные остаются в swap-группе (одна активная модель).
...
- Параллельное выполнение нескольких `Subtask` одновременно (пайплайн по-прежнему последовательный). Несколько моделей могут быть *resident* одновременно через `LocalModelBackend.Concurrent`, это не параллельные subtask.
