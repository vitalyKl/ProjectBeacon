...
export function ContextBriefPreview({ brief }: { brief: SessionBrief }) {
  ...
  const warnings = brief.lint_warnings ?? [];
  return (
    <div className="space-y-4">
      {warnings.length > 0 ? (
        <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs dark:border-amber-700 dark:bg-amber-950/30">
          <p className="font-medium text-amber-800 dark:text-amber-300">
            {t("lint.warningsTitle")}
          </p>
          <ul className="mt-1 list-disc space-y-1 pl-4 text-amber-700 dark:text-amber-400">
            {warnings.map((w) => (
              <li key={w.code}>
                <span className="font-medium">{w.title}</span>
                <span className="ml-1 text-amber-600 dark:text-amber-500">— {w.detail}</span>
              </li>
            ))}
          </ul>
        </div>
